...
return new class extends Migration
{
    public function up(): void
    {
        DB::beginTransaction();
        try {
            Schema::create('doc.sp3_pusiknas_document_officers', function (Blueprint $table) {
                ...
                $table->foreign('sp3_pusiknas_document_id', 'fk_sp3_pus_officers_doc_id')
                    ->references('id')->on('doc.sp3_pusiknas_documents')
                    ->onDelete('cascade')->onUpdate('cascade');

                $table->foreign('police_id', 'fk_sp3_pus_officers_police_id')
    ...
    public function down(): void
    {
        DB::beginTransaction();
        try {
            Schema::table('doc.sp3_pusiknas_document_officers', function (Blueprint $table) {
                $table->dropForeign('fk_sp3_pus_officers_doc_id');
                $table->dropForeign('fk_sp3_pus_officers_police_id');
            });
            Schema::dropIfExists('doc.sp3_pusiknas_document_officers');
