...
class DocumentActionController extends Controller
{
    ...
    private function getDocumentRouter($documentCategoryId, $documentId, $accidentId)
    {
        ...
        if (array_key_exists($documentCategoryId, $documentModels)) {
            $modelClass = $documentModels[$documentCategoryId];
            $relations = ['accident', 'documentCategory'];
            if (method_exists($modelClass, 'signatory')) {
                $relations[] = 'signatory';
            }
            $document = $modelClass::with($relations)
                ->where('id', $documentId)
                ->first();
        } else {
            return null;
