        ...
        switch($checkuser){
            case 2 :
                $accident = DB::table('dpb')
                            ...
                            ->paginate(10);
                $dpb = DB::select('select * from dpb left join accidents on accidents.id = dpb.accident_id left join polres on accidents.polres_id = polres.id left join polda on polda.id = polres.polda_id
                where polda.id = \''.$user->polda_id.'\'
                ');
