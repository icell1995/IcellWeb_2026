...
namespace App\Http\Controllers\IcellServices\ApiEmpBareskrim\V2\Res;

use App\Http\Controllers\Controller;
use App\Http\Resources\Res\OfficerResource;
...
class OfficerController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $registerNumber = $request->input('registerNumber');

            if($perPage > 100) {
                $perPage = 100;
            }

            $query = Officer::with(['police', 'position', 'rank'])
                        ->selectFullName()
                        ->valid()
                        ->whereHasUser()
                        ->hasDataSpecific();
            ...
            if (!empty($registerNumber)) {
                $query->where('register_number', $registerNumber);
            }

            $officers = $query->orderBy('police_id', 'asc')
