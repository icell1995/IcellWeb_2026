...
class LaporanHasilGelarPerkaraDocumentController extends Controller
{
    ...
    public function index(Request $request){
        ...
        try {
            ...
            $laporanHasilGelarPerkaraDocuments = $docService->applyIncludeLegacyDocumentFilter(
                $laporanHasilGelarPerkaraDocuments,
                false
            );

            // Filter data for 'document_date'
            $laporanHasilGelarPerkaraDocuments = $docService->applyDateRangeFilter(
                $laporanHasilGelarPerkaraDocuments,
                'released_at',
                $startReleaseDate,
                $endReleaseDate
            ...
            foreach($laporanHasilGelarPerkaraDocuments as $laporanHasilGelarPerkaraDocument){
                ...
                $responseData[$arrayKey]  = [
                    ...
                    "DORSId" => strval($dorsId),
                    "TanggalLHGP" => $laporanHasilGelarPerkaraDocument->document_date,
                    ...
                    "released_at" => $laporanHasilGelarPerkaraDocument->released_at,
                ];

                $docService->putApiSyncMoment(
                    $request, 
                    $laporanHasilGelarPerkaraDocument, 
                    get_class(new LaporanHasilGelarPerkaraDocument()),
                    $this->tableSchemaName . 'laporan_hasil_gelar_perkara_documents', 
                    $mode
                );
