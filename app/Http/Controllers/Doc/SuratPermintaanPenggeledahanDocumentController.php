...
class SuratPermintaanPenggeledahanDocumentController extends Controller
{
    ...
    public function store()
    {
       ...
       // dd($data);
        ...
        $validator = Validator::make($data, [
            ...
            'daftar_penggeledahan'       => 'required|array',
            'alamatPenggeledahan'      => 'required|string|max:255',
        ...
        try {
            ...
            // Create document
            $document = SuratPermintaanPenggeledahanDocument::create([
                ...
                'no_spdp'                                               => $noSpdp,
                'daftar_penggeledahan'                                  => $data['daftar_penggeledahan'] ?? ($data['jenisPenggeledahan'] ?? []),
                'alamat_penggeledahan'                                  => $data['alamatPenggeledahan'],
    ...
    {
        $accidentId = htmlspecialchars(request()->query('accident_id'));
        $document = SuratPermintaanPenggeledahanDocument::with([
        ...
        return view('docs.surat-permintaan-penggeledahan-document.show', compact(
            'accidentId',
            'accident',
            'document'
        ));
    ...
    {
        $accidentId = htmlspecialchars(request()->query('accident_id'));
        $document   = SuratPermintaanPenggeledahanDocument::where('id', $id)->firstOrFail();
    ...
    public function download($id)
    {
        ...
        $templatePath = public_path('word-template/surat_permintaan_penggeledahan.docx');
        if (!file_exists($templatePath)) {
            $templatePath = base_path('public/word-template/surat_permintaan_penggeledahan.docx');
        }
        ...
        $filename = public_path('generate/' . $document->id . ' - Surat Permintaan Izin Penggeledahan - ' . ($accident->polres->full_name ?? 'Polres') . '.docx');
    ...
    public function validateRequestForm()
    {
        ...
        $validator = Validator::make($data, [
            ...
            'tanggal_sprindik'         => 'required|string|max:255',
            'documentNumber'           => 'required|string|max:255',
            'documentDate'             => 'required|string|max:255',
            'no_spdp'                  => 'required|string|max:255',
            'daftar_penggeledahan'     => 'required|array',
