...
// SURAT PERINTAH PENYELIDIKAN (SPRINLIDIK)
class InvestigationWarrantController extends Controller
{
    public function index(Request $request){
        ...
        try {
            ...
            if (!empty($startDocumentDate) && !empty($endDocumentDate)) {
                $getData = $getData->whereBetween('TanggalSprinlidik', [date('Y-m-d H:i:s', strtotime($startDocumentDate)), date('Y-m-d H:i:s', strtotime($endDocumentDate))]);
            } else if (!empty($startDocumentDate)) {
                $getData = $getData->where('TanggalSprinlidik', '>=', date('Y-m-d H:i:s', strtotime($startDocumentDate)));
            } else if (!empty($endDocumentDate)) {
                $getData = $getData->where('TanggalSprinlidik', '<=', date('Y-m-d H:i:s', strtotime($endDocumentDate)));
            }
    ...
    private function collectionData(){
        ...
        for ($date = $start_date; $date <= $end_date; $date->modify('+1 day')) {
            $data[] = [
                    "Id" => Str::uuid(),
                    "NoSurat" => "SURAT/SPRINLIDIK/" . $no . "/2022",
                    "TanggalSprinlidik" => $date->format('Y-m-d H:i:s'),
