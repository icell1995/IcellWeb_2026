        ...
        <div class="radius-card mt-4 w-60">
            <form target="_blank" method="POST">
                ...
                <div class="col-lg-12 col-md-12 col-xs-12 col-sm-12 text-center">
                    ...
                    <h4 class="pt-1 pb-2 font-weight-bolder">NOMOR : {{$investigationOrderLetter->letter_number}}</h4>
                </div>
                ...
                <div class="row col-lg-12 col-md-12 col-sm-12 col-12 mb-3">
                    ...
                    <div class="col-lg-9 col-md-9 col-sm-9 col-9 p-0">
                        <textarea class="form-control inline-block txtarea" name="" id="" disabled>@php $xNo = 1; @endphp @foreach($investigationOrderLetter->leaderOfficers as $officer){{$xNo}}. Nama : {{$officer->first_name . ' ' . $officer->last_name}}&#13;&#10;Pangkat/NRP : {{$officer->rank_id . '/' . $officer->id}}&#13;&#10;Jabatan : {{$officer->position_short_name}}&#13;&#10;@php $xNo++; @endphp @endforeach @foreach($investigationOrderLetter->officers as $officer){{$xNo}}. Nama : {{$officer->first_name . ' ' . $officer->last_name}}&#13;&#10;Pangkat/NRP : {{$officer->rank_id . '/' . $officer->id}}&#13;&#10;Jabatan : {{$officer->position_short_name}}&#13;&#10;@php $xNo++; @endphp @endforeach</textarea>
                    </div>
                </div>
                ...
                <div class="row col-lg-12 col-md-12 col-sm-12 col-12 mb-3">
                    ...
                    <div class="col-lg-9 col-md-9 col-sm-9 col-9 p-0">
                        <textarea class="form-control inline-block txtarea"
                            name="" disabled>1. Melakukan penyidikan tindak pidana kasus kecelakaan lalu lintas yang terjadi pada Hari {{Carbon\Carbon::parse($accident->accident_date)->locale('id')->translatedFormat('l')}} Tanggal {{Carbon\Carbon::parse($accident->accident_date)->locale('id')->translatedFormat('d F Y')}} sekira pukul {{Carbon\Carbon::parse($accident->accident_time)->locale('id')->translatedFormat('H:i')}} di {{$accident->road_name}}&#13;&#10;2. Membuat rencana penyidikan&#13;&#10;3. Melaporkan setiap perkembangan pelaksanaan penyidik tindak pidana tersebut kepada {{$investigationOrderLetter->authorizedSignatories->first()->position_id}} selaku Penyidik&#13;&#10;4. Surat Perintah ini berlaku sejak tanggal dikeluarkan</textarea>
                    </div>
                </div>

                <button type="button" class="btn btn-primary float-right mr-4">
                    <a href="{{route('investigation-order-letter.print', ['id'=> $investigationOrderLetterId, 'accident_id' => $accident->id])}}" style="color: #ffffff">Generate</a>
                </button>
