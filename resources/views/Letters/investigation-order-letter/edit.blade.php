...
<div class="content col-xs-12 col-md-12 col-lg-12 col-sm-12">
    ...
    <div class="box">
        <div class="box-header">
            <h5 class="fw-bold text-blue-dark">Edit Surat Perintah Penyidikan (SPRINDIK)</h5>
        </div>

        <form action="{{ route('investigation-order-letter.update', ['id'=> $investigationOrderLetterId,'accident_id' => $accidentId]) }}" method="POST" enctype="multipart/form-data">
            @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group" style="padding: 0px">
                            <input id="letter_number" type="text"
                                ...
                                value="{{ $investigationOrderLetter->letter_number }}" required placeholder="Masukkan Nomor Surat Sprindik">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group" style="padding: 0px">
                            <input class="form-control datepicker" id="start_date" name="start_date"
                                placeholder="DD/MM/YYYY" autocomplete="off" value="{{Carbon\Carbon::parse($investigationOrderLetter->start_date)->format('Y-m-d')}}" data-provide="datepicker">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group" style="padding: 0px">
                            <input class="form-control datepicker" id="end_date" name="end_date"
                                placeholder="DD/MM/YYYY" autocomplete="off" value="{{Carbon\Carbon::parse($investigationOrderLetter->end_date)->format('Y-m-d')}}" data-provide="datepicker">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group" style="padding: 0px">
                            <select class="form-control" name="authorized_signatory" id="authorized_signatory">
                                ...
                                <option value="{{$data->id}}" @if($investigationOrderLetterAuthorizedSignatory->id == $data->id){{'selected'}}@endif>{{$data->register_number . ' - ' . $data->full_name . ' | ' . $data->position_id}}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-5">
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr class="text-center">
                                    <th>
                                    <input class="form-check-input" type="checkbox" value="{{$data->id}}" id="law{{$data->id}}" name="laws[]" {{($investigationOrderLetter->laws->where('id', '=', $data->id)->count() == 1) ? 'checked' : ''}}>
                                    </th>
                                    <td>{{$data->law}}</td>
                                    <td>{{$data->chapter}}</td>
                                    <td>{{$data->verse}}</td>
                        ...
                        <div class="input-group" style="padding: 0px">
                            <select class="form-control" name="personnel_leader" id="personnel_leader">
                                @foreach($personnelLeaders as $data)
                                    <option value="{{$data->id}}" {{($investigationOrderLetter->leaderOfficers->where('id', '=', $data->id)->count() == 1) ? 'selected' : '' }}>{{$data->id . ' - ' . $data->first_name . ' ' . $data->last_name . ' | ' . $data->sebagai_kepala}}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group" style="padding: 0px">
                            ...
                            <div class="col-sm-12 col-md-12" id="add_personel_penyidikan">
                                ...
                                @foreach($investigationOrderLetter->officers as $row)
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    ...
                                    <tr class="text-center">
                                        <th>
                                            <input class="form-check-input" type="checkbox" value="{{$data->id}}" id="personnel{{$data->id}}" name="personnel[]" @if($investigationOrderLetter->officers->where('id', $data->id)->count() == 1){{'checked'}}@endif>
                                        </th>
                                        <td>{{$data->id}}</td>
