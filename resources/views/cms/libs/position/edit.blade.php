...
<div class="box">
    <div class="box-header">
        <h4 class="fw-bold text-blue-dark">Edit Jabatan</h4>
    ...
    <div class="boxy-body">
        <form
            action="{{ route('cms.libs.position.edit', ['id'=> $position->id]) }}"
            method="POST" enctype="multipart/form-data" id="positionForm">
            @csrf
            @method('PUT')
            <input id="oldId" type="hidden" class="form-control" name="oldId"
                        value="{{ old('oldId', $position->id) }}">
            ...
            <div class="input-group row mb-3 ms-0">
                ...
                <div class="col-lg-10 col-md-10 col-sm-12 col-12 d-flex align-self-center">
                    <input id="id" type="text" class="form-control @error('id') is-invalid @enderror" name="id"
                        value="{{ old('id', $position->id) }}" required
                        placeholder="ID">
                    @error('id')
            ...
            <div class="input-group row mb-3 ms-0">
                ...
                <div class="col-lg-10 col-md-10 col-sm-12 col-12 d-flex align-self-center">
                    <input id="empId" type="text" class="form-control @error('empId') is-invalid @enderror" name="empId"
                        value="{{ old('empId', $position->emp_id) }}"
                        placeholder="EMP ID">
                    @error('empId')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                    ...
                    <div class="d-flex">
                        <div class="form-check mx-1">
                            <input class="form-check-input" type="radio" id="typePoliceEmployment" name="employmentType"
                                value="1"
                                @if (old('employmentType', $position->employment_type_id) == 1 || empty(old('employmentType', $position->employment_type_id))) checked @endif>
                        ...
                        <div class="form-check mx-1">
                            <input class="form-check-input" type="radio" id="typeCivilEmployment" name="employmentType"
                                value="2"
                                @if (old('employmentType', $position->employment_type_id) == 2)
                                        checked @endif>
                    ...
                    <select class="form-control select2" name="positionCluster" id="positionCluster">
                        ...
                        @foreach ($positionClusters as $positionCluster)
                            <option value="{{ $positionCluster->id }}"
                                {{ (old('positionCluster', $position->position_cluster_id) == $positionCluster->id) ? 'selected' : '' }}>
                    ...
                    <select class="form-control select2" name="police" id="police">
                        ...
                        @foreach ($polices as $police)
                            <option value="{{ $police->id }}"
                                {{ (old('police', $position->police_id) == $police->id) ? 'selected' : '' }}>
                ...
                <div class="col-lg-10 col-md-10 col-sm-12 col-12 d-flex align-self-center">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="isActive"
                            ...
                            @if (old('isActive', $position->is_active) == 'true' || old('isActive', $position->is_active) == true) checked @endif>
                        <label for="isActive">
                            Aktif
                        </label>
                    </div>
                ...
                <div class="col-lg-10 col-md-10 col-sm-12 col-12 d-flex align-self-center">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="isCanSignatory"
                            ...
                            @if (old('isCanSignatory', $position->is_can_signatory) == 'true' || old('isCanSignatory', $position->is_can_signatory) == true) checked @endif>
