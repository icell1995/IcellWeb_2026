...
<script>
    ...
    const APP_CONFIG = {
        ...
        */
        ...
        let fullText = '';
        for (let pageNum = 1; pageNum <= this.pdfDoc.numPages; pageNum++) { const page=await this.pdfDoc.getPage(pageNum); const
            textContent=await page.getTextContent(); const pageText=textContent.items.map(item=> item.str).join(' ');
            fullText += pageText + '\n';
            }
...
async runValidation() {
        ...
        const result = await this.validator.searchText(value);
        this.results[field] = {
        ...result,
...
<input class="form-check-input my-auto reject-reason-checkbox" type="checkbox" value="${reason}"
                        ...
                        <label class="form-check-label" for="reject-reason-${idx}">
                            ${reason}
                        </label>
                        </div>
                        `).join('');
