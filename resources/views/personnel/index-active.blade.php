                        ...
                        <tbody>
                            ...
                            @if (!empty($users))
                                @foreach ($users as $user)
                                    @php
                                        $positionClusterId = $user->officer->position->position_cluster_id ?? null;
                                    @endphp

                                    <tr class="{{($user->officer->status == 'ASSISTANCE') ? 'table-primary' : ((!isset($user->officer->position->name) || !isset($user->officer->rank->name)) ? 'table-danger' : '')}}">
                                        <td class="text-center align-middle">
                                            {{ $iteration }}
                                        </td>
                                        <td class="text-center align-middle">
                                            {{ $user->officer->full_name }} {{($user->officer->status == 'ASSISTANCE') ? ', (BKO)' : ''}}
                                            @if(!isset($user->officer->position->name) || !isset($user->officer->rank->name))
                                                <h6><span class="badge bg-danger">Data Belum Lengkap</span></h6>
                                            @else
                                                <h6><span class="badge bg-primary">Data Lengkap</span></h6>
                                            @endif
                                            
                                            @if($user->officer->is_certificate_exists == false)
                                                <h6><span class="badge bg-danger">Belum Memiliki Sertifikat</span></h6>
                                            @else
                                                <h6><span class="badge bg-secondary">Memiliki Sertifikat</span></h6>
                                            @endif
                                        </td>
                                        <td class="text-center align-middle">
