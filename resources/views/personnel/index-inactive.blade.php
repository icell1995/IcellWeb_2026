...
@section('content')
    ...
    <div class="box">
        <div class="box-header">
            <h3 class="fw-bold text-blue-dark"> Daftar Anggota Tidak Aktif- SAT LANTAS
                    ...
                    <tbody>
                        ...
                        @if (!empty($users))
                            @foreach ($users as $user)
                                @if ($user->officer->status == 'EXIT' || $user->officer->status == 'RETIRE')
                                    <tr>
                                        ...
                                        <td class="text-center align-middle">
                                            ...
                                            @else
                                                @php 
                                                    ...
                                                    if($user->officer->status == 'EXIT'){
                                                        $status = 'Keluar Gakkum Lantas';
                                                    }else if($user->officer->status == 'RETIRE'){
                                                        $status = 'Pensiun';
                                                    }
                                                @endphp
                                                <div class="d-grid gap-2">
                                                    <button type="button" class="btn btn-sm btn-warning"
                                                        disabled><b>{{ $status }}</b></button>
                                                </div>
                                            @endif
                                        </td>
                                        <td class="text-center align-middle">
                                            ...
                                            @if (( empty(Auth::user()->police_id)) || 
                                                ( !empty(Auth::user()->police_id) && $positionClusterId != '6' || !empty(Auth::user()->police_id) && $positionClusterId != '12'))
                                            ...
                                            @if (Auth::user()->hasPermission('personnel.U'))
                                                <a href="{{ route('personnel.move', ['id' => $user->id, 'policeId' => $user->police_id]) }}"
                                                    ...
                                                    <i class="bi bi-person-plus"></i> Restore
                                                </a>
                                            @endif
                                        </td>
                                    </tr>
