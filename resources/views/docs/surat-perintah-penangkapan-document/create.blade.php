...
@push('style')
    ...
    <style>
        /* Seperti pola perpanjangan lanjutan: Select2 di input-group mengisi sisa lebar */
...
@section('content')
    ...
    <div class="box">
        ...
        <div class="box-body">
            <form method="POST"
                action="{{ route('doc.surat-perintah-penangkapan-document.store', ['accident_id' => $accidentId, 'document_category_id' => '0301']) }}">
                @csrf
                <input type="hidden" name="accident_id" value="{{ $accidentId }}">

                <div class="input-group row mb-3 ms-0">
                    <label class="fw-bold col-sm-2 col-form-label" for="accidentNumber">Nomor LP<span
                ...
                <div class="input-group row mb-3 ms-0">
                    <label class="fw-bold col-sm-2 col-form-label" for="documentNumber">Nomor Surat<span
                            class="text-danger fs-5">*</span></label>
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12 d-flex align-self-center">
                ...
                <div class="input-group row mb-3 ms-0">
                    <label class="fw-bold col-sm-2 col-form-label">Tanggal Surat<span
                            class="text-danger fs-5">*</span></label>
                ...
                <div class="input-group row mb-3 ms-0">
                    <label class="fw-bold col-sm-2 col-form-label" for="suratPerintahPenyidikanDocument">Surat Perintah
                        Penyidikan<span class="text-danger fs-5">*</span></label>
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        @php
                        ...
                        <select class="form-control select2" name="suratPerintahPenyidikanDocument"
                            id="suratPerintahPenyidikanDocument" required>
                            <option value="">--Pilih No Surat Perintah Penyidikan--</option>
                            @foreach ($suratPerintahPenyidikanDocuments as $d)
                                <option value="{{ $d->id }}" {{ (string) $sprindikSelected === (string) $d->id ? 'selected' : '' }}>
                                    {{ $d->document_number }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="input-group row mb-3 ms-0">
                    <label class="fw-bold col-sm-2 col-form-label" for="suratKetetapanTentangPenetapanTersangkaDocument">Surat
                        Ketetapan tentang Penetapan Tersangka<span class="text-danger fs-5">*</span></label>
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        @php
                            $sketSelected = old('suratKetetapanTentangPenetapanTersangkaDocument');
                        @endphp
                        <select class="form-control select2" name="suratKetetapanTentangPenetapanTersangkaDocument"
                            id="suratKetetapanTentangPenetapanTersangkaDocument" required>
                            ...
                            @foreach ($suratKetetapanTentangPenetapanTersangkaDocuments as $d)
                                <option value="{{ $d->id }}" {{ (string) $sketSelected === (string) $d->id ? 'selected' : '' }}>
                                    {{ $d->document_number }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="input-group row mb-3 ms-0">
                    <label class="fw-bold col-sm-2 col-form-label" for="suratPerintahTugasDocument">Surat Perintah
                        Tugas<span class="text-danger fs-5">*</span></label>
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        @php
                            $sptSelected = old('suratPerintahTugasDocument');
                        @endphp
                        <select class="form-control select2 @error('suratPerintahTugasDocument') is-invalid @enderror"
                            ...
                            @foreach ($suratPerintahTugasDocuments as $d)
                                <option value="{{ $d->id }}" {{ (string) $sptSelected === (string) $d->id ? 'selected' : '' }}>
                                    {{ $d->document_number }}
                                </option>
                            @endforeach
                        </select>
                ...
                @php
                    $s22RefsReady = filled(old('suratPerintahPenyidikanDocument'))
                        && filled(old('suratKetetapanTentangPenetapanTersangkaDocument'))
                        && filled(old('suratPerintahTugasDocument'));
                @endphp
                <div class="input-group row mb-3 ms-0">
                    <label class="fw-bold col-sm-2 col-form-label" for="suspect">Identitas Tersangka<span
                            class="text-danger fs-5">*</span></label>
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        <select class="form-control select2 @error('suspect') is-invalid @enderror" name="suspect"
                            id="suspect" required @unless ($s22RefsReady) disabled @endunless>
                            <option value="">--Pilih Tersangka--</option>
                            @foreach ($suspects as $s)
                                <option value="{{ $s->id }}" {{ (string) old('suspect') === (string) $s->id ? 'selected' : '' }}>
                ...
                <div class="input-group row mb-3 ms-0">
                    <label class="fw-bold col-sm-3 col-form-label" for="spp_officerLeader">Ketua Tim Penyidik <span
                            class="text-danger fs-5">*</span></label>
                    <div class="col-lg-9">
                                ...
                                <tbody>
                                    @foreach ($kepadaInternalRows ?? [] as $o)
                                        ...
                                        <tr class="text-center">
                                            ...
                                            <td>{{ $policeName }}</td>
                                            <td>
                                                <input type="hidden" name="internalOfficers[]"
                                                    value="{{ $o->register_number }}">
                                                <button class="btn btn-danger btn-sm spp_deleteInternalOfficer"
                                                    type="button"><i class="bi bi-trash"></i> Hapus</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                ...
                <div class="input-group row mb-3 ms-0">
                    <label class="fw-bold col-sm-2 col-form-label" for="signatoryOfficerId">Yang Menandatangani<span
                            class="text-danger fs-5">*</span></label>
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        @php
                        ...
                        <select class="form-control select2 @error('signatoryOfficerId') is-invalid @enderror"
                            ...
                            @foreach (($authorizedSignatories ?? collect()) as $o)
                                ...
                                <option value="{{ $o->id }}" {{ (string) $sigSelected === (string) $o->id ? 'selected' : '' }}>
                                    {{ ! empty($o->register_number) ? $o->register_number : '' }} -
                                    {{ $o->full_name ?? $o->name ?? '' }} | {{ $positionName }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="input-group row mb-3 ms-0">
                    <label class="fw-bold col-sm-2 col-form-label" for="submittedOfficerId">Yang Menyerahkan<span
                            class="text-danger fs-5">*</span></label>
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        @php
                ...
                <div class="input-group row mb-3 ms-0">
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        <input type="text" class="form-control spp-date @error('validUntilDate') is-invalid @enderror"
                            ...
                            value="{{ old('validUntilDate') }}" autocomplete="off" readonly>
                ...
                <div class="input-group row mb-3 ms-0">
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        <input type="text" class="form-control spp-date @error('handoverDate') is-invalid @enderror"
                            ...
                            value="{{ old('handoverDate') }}" autocomplete="off" readonly>
                        @error('handoverDate')
                            <span class="invalid-feedback d-block" role="alert"><strong>{{ $message }}</strong></span>
                        @enderror
                    </div>
...
@push('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            var sppDpOpts = { format: 'yyyy-mm-dd', autoclose: true };
            $('.spp-date').datepicker(sppDpOpts);
            $('.spp-date').keydown(function(e) {
                e.preventDefault();
                return false;
            });
