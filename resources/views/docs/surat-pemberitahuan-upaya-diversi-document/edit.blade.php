@php
    $_title = 'Edit Surat Pemberitahuan Upaya Diversi (SPUD)';
@endphp
...
@section('content')
    ...
    <div class="box">
        <div class="box-header">
            <h5 class="fw-bold text-blue-dark">Edit Surat Pemberitahuan Upaya Diversi (SPUD)</h5>
        ...
        <div class="box-body">
            <form
                action="{{ route('doc.surat-pemberitahuan-upaya-diversi-document.update', ['accident_id' => $accidentId, 'id' => $spudDocument->id]) }}"
                ...
                <div class="input-group row mb-3 ms-0">
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        <input id="accidentNumber" type="text"
                            ...
                            name="accidentNumber" value="{{ $accident->no_lp }}" required readonly>
                    </div>
                </div>
                ...
                <div class="input-group row mb-3 ms-0">
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        <input id="documentNumber" type="text"
                            ...
                            name="documentNumber" value="{{ old('documentNumber', $spudDocument->document_number) }}" required
                        ...
                        <select class="form-control select2" name="documentClassification" id="documentClassification">
                            ...
                            @foreach ($documentClassifications as $documentClassification)
                                <option value="{{ $documentClassification->id }}"
                                    {{ old('documentClassification', $spudDocument->document_classification_id) == $documentClassification->id ? 'selected' : '' }}>
                ...
                <div class="input-group row mb-3 ms-0">
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        <select class="form-control select2" name="suratPerintahPenyidikanDocument" id="suratPerintahPenyidikanDocument">
                            ...
                            @foreach ($suratPerintahPenyidikanDocuments as $spDoc)
                                <option value="{{ $spDoc->id }}"
                                    {{ old('suratPerintahPenyidikanDocument', $spudDocument->surat_perintah_penyidikan_document_id) == $spDoc->id ? 'selected' : '' }}>
                                    {{ $spDoc->document_number }}
                                </option>
                            @endforeach
                        </select>
                ...
                {{-- No SPDP --}}
                <div class="input-group row mb-3 ms-0">
                    <label class="fw-bold col-sm-2 col-form-label" for="suratPemberitahuanDimulainyaPenyidikanDocument">No SPDP<span class="text-danger fs-5">*</span></label>
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        <select class="form-control select2" name="suratPemberitahuanDimulainyaPenyidikanDocument" id="suratPemberitahuanDimulainyaPenyidikanDocument">
                            ...
                            @foreach ($suratPemberitahuanDimulainyaPenyidikanDocuments as $spdpDoc)
                                <option value="{{ $spdpDoc->id }}"
                                    {{ old('suratPemberitahuanDimulainyaPenyidikanDocument', $spudDocument->surat_pemberitahuan_dimulainya_penyidikan_document_id) == $spdpDoc->id ? 'selected' : '' }}>
                        ...
                        <select class="form-control select2" name="suspectId" id="suspectId">
                            ...
                            @foreach ($suspects as $suspect)
                                @php
                                    ...
                                    if ($spudDocument->suspect_id == $suspect->id && !empty($spudDocument->messages['suspect_age'])) {
                                        $suspectAge = $spudDocument->messages['suspect_age'];
                                ...
                                @endphp
                                <option value="{{ $suspect->id }}" {{ old('suspectId', $spudDocument->suspect_id) == $suspect->id ? 'selected' : '' }}>
                        ...
                        <select class="form-control select2" name="prosecutor" id="prosecutor">
                            ...
                            @foreach ($prosecutors as $prosecutor)
                                <option value="{{ $prosecutor->id }}"
                                    {{ old('prosecutor', $spudDocument->prosecutor_id) == $prosecutor->id ? 'selected' : '' }}>
                                    {{ $prosecutor->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="input-group row mb-3 ms-0">
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        <input id="appendix" type="text"
                            ...
                            value="{{ old('appendix', $spudDocument->appendix) }}" required>
                ...
                <div class="input-group row mb-3 ms-0">
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        <input class="form-control" id="documentDate" name="documentDate"
                            ...
                            value="{{ old('documentDate', $spudDocument->document_date ? date('Y-m-d', strtotime($spudDocument->document_date)) : '') }}"
                ...
                {{-- Penandatangan Surat --}}
                @php
                    $currentSignatoryId = $spudDocument->suratPemberitahuanUpayaDiversiDocumentOfficers->first()->register_number ?? null;
                    // match by officer register number or id
                    $selectedSignatory = $authorizedSignatories->where('register_number', $currentSignatoryId)->first();
                    $selectedSignatoryId = $selectedSignatory->id ?? null;
                @endphp
                <div class="input-group row mb-3 ms-0">
                        ...
                        <select class="form-control select2" name="signatory" id="signatory">
                            ...
                            @foreach ($authorizedSignatories as $data)
                                ...
                                <option value="{{ $data->id }}"
                                    {{ old('signatory', $selectedSignatoryId) == $data->id ? 'selected' : '' }}>
                        ...
                        <select class="form-control select2" name="court" id="court">
                            ...
                            @foreach ($courts as $court)
                                <option value="{{ $court->id }}"
                                    {{ old('court', $spudDocument->court_id) == $court->id ? 'selected' : '' }}>
                                    {{ $court->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="input-group row mb-3 ms-0">
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-12 col-12">
                        <div id="carbonCopiesContainer">
                            @if (!empty($spudDocument->carbon_copies))
                                @foreach ($spudDocument->carbon_copies as $carbonCopy)
                                    <div class="input-group mb-2">
                                        <input type="text" class="form-control" name="carbonCopies[]"
                                            value="{{ $carbonCopy }}">
                                        <div class="input-group-append">
                                            <button class="btn btn-outline-danger removeCarbonCopiesButton"
                ...
                <div class="text-center">
                    <button type="submit" class="btn btn-dark-blue me-2" id="suratPemberitahuanUpayaDiversiFormSubmit">
