      ...
      <fieldset id="search-filter" class="border rounded-3 p-3">
        <form id="filter-form" class="row mt-2 search">
          ...
          <div class="col-lg-4 col-md-4 col-sm-12 col-12 mb-3">
            ...
            @else
              <select id="polda" name="polda" class="form-select select2">
                <option value="">Semua Polda</option>
                @foreach ($poldas as $p)
                  @if(in_array($p->name, ['POLDA XE', 'PJR INDUK KORLANTAS', 'PUSDIKLANTAS', 'DIT GAKKUM KORLANTAS']))
                    @continue
                  @endif
          ...
          <div class="col-lg-4 col-md-4 col-sm-12 col-12 mb-3">
            ...
            @elseif($poldaLocked)
              <select id="polres" name="polres" class="form-select select2">
...
@push('script')
  ...
  <script>
    ...
    function reloadTable(){
      // Khusus role 1 & 2: jangan load kalau filter belum cukup
      if ((ROLE_ID === 1 || ROLE_ID === 2) && !role1HasSufficientFilters()){
        destroyTableIfAny();
        $('#role1-info').show();
        return;
      }
    ...
    $(function(){
      ...
      // Load awal:
      // - Role 1 & 2: JANGAN auto-load (biarkan kosong & tampilkan info)
      // - Role selain 1 & 2: auto-load seperti biasa
