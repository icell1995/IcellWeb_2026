                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6 col-md-6 col-sm-12 col-12 pt-2">
                                <div class="mb-3">
                                    ...
                                    @endphp
                                    @if ($roleId == 1)
                                        <select id="polda_id" name="polda_id" class="form-control @error('polda_id') is-invalid @enderror">
                                    ...
                                    @elseif ($roleId == 3 && !is_null($userPoldaId) && $userPoldaId !== '')
                                        <select id="polda_id" name="polda_id" class="form-control" disabled>
                                            @foreach ($polda as $poldas)
                                        ...
                                        <input type="hidden" name="polda_id" value="{{ $userPoldaId }}">
                                    @elseif ($roleId == 3 && (is_null($userPoldaId) || $userPoldaId === ''))
                                        <select id="polda_id" name="polda_id" class="form-control" disabled>
                                            @foreach ($polda as $poldas)
                                                @if ($poldas->id == $userPoldaId)
                            ...
                            <div class="col-lg-6 col-md-6 col-sm-12 col-12 pt-2">
                                <div class="mb-3">
                                    ...
                                    @endphp
                                    @if ($roleId == 1)
                                        <select id="polres_id" name="polres_id" class="form-control @error('polres_id') is-invalid @enderror">
                                    ...
                                    @elseif ($roleId == 3 && (is_null($userPolresId) || $userPolresId === ''))
                                        <select id="polres_id" name="polres_id" class="form-control @error('polres_id') is-invalid @enderror">
                                            <option value="-">- Pilih Polres -</option>
                                            @foreach ($polres as $polress)
                                    ...
                                    @elseif (($roleId == 3 && !is_null($userPolresId) && $userPolresId !== '') || $roleId == 4)
                                        ...
                                        @endphp
                                        <select id="polres_id" name="polres_id" class="form-control" disabled>
                                            <option value="{{ $userPolresId }}" selected>{{ $namaPolres }}</option>
                                        </select>
                                        <input type="hidden" name="polres_id" value="{{ $userPolresId }}">
                                    @endif
                                </div>
                            </div>
    ...
    (function ($) {
        ...
        $(function () {
            ...
            if (!initPolda || initPolda === '-') {
                ...
                if (isLevel3Polda) {
                    setTimeout(function() { $polres.prop('disabled', false); }, 300);
                }
            }
        });
                    ...
                    <div class="container-fluid mt-3">
                      <div style="overflow-x:auto; width:100%">
                        <table id="dataTable" class="display table table-bordered table-striped w-100">
                            <thead>
                                <tr>
                                    <th colspan="${window.role_id == 1 ? 10 : 8}" class="periode-lalu text-center" style="background-color: #99ceff">Periode Lalu</th>
                                    <th colspan="${window.role_id == 1 ? 8 : 6}" class="periode-ini text-center text-white" style="background-color: #0000cc">Periode Ini</th>
                                </tr>
                                <tr>
                                    ...
                                    ${window.role_id == 1 ? `<th rowspan="2" class="text-center" style="background-color: #99ceff">LP IRSMS<br> (TGL KEJADIAN)</th>` : ''}
                                    ...
                                    ${window.role_id == 1 ? `<th rowspan="2" class="text-center" style="background-color: #99ceff">%</th>` : ''}
                                    ${window.role_id == 1 ? `<th rowspan="2" class="text-center text-white" style="background-color: #0000cc">LP IRSMS<br> (TGL KEJADIAN)</th>` : ''}
                                    ...
                                    ${window.role_id == 1 ? `<th rowspan="2" class="text-center text-white" style="background-color: #0000cc;">%</th>` : ''}
                                </tr>
                                <tr>
                    ...
                    data.forEach((row, i) => {
                        html += `
                            <tr>
                                ...
                                ${window.role_id == 1 ? `<td class="text-center fw-bold" style="background-color: #99ceff">${Number(row.total_laka_lalu ?? 0).toLocaleString('id-ID')}</td>` : ''}
                                ...
                                ${window.role_id == 1 ? `<td class="text-center" style="background-color: #99ceff">${Number(row.persen_selra_lalu ?? 0).toLocaleString('id-ID')}%</td>` : ''}
                                ${window.role_id == 1 ? `<td class="text-center text-white" style="background-color: #0000cc;">${Number(row.total_laka_ini ?? 0).toLocaleString('id-ID')}</td>` : ''}
                                ...
                                ${window.role_id == 1 ? `<td class="text-center text-white" style="background-color: #0000cc;">${Number(row.persen_selra_ini ?? 0).toLocaleString('id-ID')}%</td>` : ''}
                            </tr>
                        `;
                    ...
                    html += `
                        ...
                        <tfoot>
                            <tr>
                                ...
                                ${window.role_id == 1 ? `<th class="text-center" style="background-color: #99ceff">${Number(totalLalu.laka).toLocaleString('id-ID')}</th>` : ''}
                                ...
                                ${window.role_id == 1 ? `<th class="text-center" style="background-color: #99ceff">${Number(persenLalu).toLocaleString('id-ID')}%</th>` : ''}
                                ${window.role_id == 1 ? `<th class="text-center text-white" style="background-color: #0000cc;">${Number(totalIni.laka).toLocaleString('id-ID')}</th>` : ''}
                                ...
                                ${window.role_id == 1 ? `<th class="text-center text-white" style="background-color: #0000cc;">${Number(persenIni).toLocaleString('id-ID')}%</th>` : ''}
                            </tr>
                        </tfoot>
                    </table>`;
                    ...
                    $(document).ready(function () {
                        $('#dataTable').DataTable({
                            ...
                            },
                            {
                                ...
                                // (opsional) kalau sebelumnya pakai customizeData untuk kolom persen, biarkanâ€”jangan ubah koma di sana
