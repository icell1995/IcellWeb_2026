        ...
        <div class="box-header">
            <div class="row">
                ...
                <div class="col-lg-3 col-md-3 col-sm-12 col-12">
                    <form id="update-selra-form">
                        @csrf
                        <input id="accident_id_selra" name="accident_id_selra" type="text" value="{{ $id }}"
                            hidden>
                        <div class="form-group row">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-12">
                                @foreach ($selra_flag as $selra_flags)
                                    <select id="selra_flag" name="selra_flag"
                                        class="form-select @error('selra_flag') is-invalid @enderror">
                                        {{-- <option value="">Status Selra</option> --}}
                                        @php 
                                            $suratPemberitahuanDimulainyaPenyidikanDocumentsCount = $countAccidentDocuments['suratPemberitahuanDimulainyaPenyidikanDocuments']['count'] ?? 0;
                                            $suratPerintahPenyelidikanDocumentsCount = $countAccidentDocuments['suratPerintahPenyelidikanDocuments']['count'] ?? 0;
                                        @endphp

                                        @foreach ($selra as $selras)
                                            @if ($selras->id == 'S0101' && $suratPemberitahuanDimulainyaPenyidikanDocumentsCount > 0)
                                                <option value="{{ $selras->id }}"
                                                    {{ $selras->id == $selra_flags->selra_flag ? 'selected' : '' }}>
                                                    {{ $selras->name }}</option>
                                            @elseif($selras->id == 'S0102' && $suratPemberitahuanDimulainyaPenyidikanDocumentsCount > 0)
                                                <option value="{{ $selras->id }}"
                                                    {{ $selras->id == $selra_flags->selra_flag ? 'selected' : '' }}>
                                                    {{ $selras->name }}</option>
                                            @elseif($selras->id == 'S0103' && $suratPemberitahuanDimulainyaPenyidikanDocumentsCount > 0)
                                                <option value="{{ $selras->id }}"
                                                    {{ $selras->id == $selra_flags->selra_flag ? 'selected' : '' }}>
                                                    {{ $selras->name }}</option>
                                            @elseif($selras->id == 'S0104' && $suratPerintahPenyelidikanDocumentsCount > 0)
                                                <option value="{{ $selras->id }}"
                                                    {{ $selras->id == $selra_flags->selra_flag ? 'selected' : '' }}>
                                                    {{ $selras->name }}</option>
                                            {{--@elseif($selras->id == 'S0106' && $suratPerintahPenyelidikanDocumentsCount > 0)
                                                <option value="{{ $selras->id }}"
                                                    {{ $selras->id == $selra_flags->selra_flag ? 'selected' : '' }}>
                                                    {{ $selras->name }}</option> RJ WILL DELETED SOON--}} 
                                            @elseif($selras->id == 'S0108' && $suratPerintahPenyelidikanDocumentsCount > 0)
                                                <option value="{{ $selras->id }}"
                                                    {{ $selras->id == $selra_flags->selra_flag ? 'selected' : '' }}>
                                                    {{ $selras->name }}</option>
                                            @elseif($selras->id == 'S0107')
                                                <option value="{{ $selras->id }}"
                                                    {{ $selras->id == $selra_flags->selra_flag ? 'selected' : '' }}>
                                                    {{ $selras->name }}</option>
                                            @endif
                    ...
                    <form id="update-state-selra-form">
                        @csrf
                        <input id="accident_id_selra" name="accident_id_selra" type="text" value="{{ $id }}"
                            hidden>
                        @foreach ($state_selra_flag as $state_selra_flags)
                            <div class="input-group" id="check_state_selra_flag" name="check_state_selra_flag"
                                style="display: none">
                                <input value={{ $state_selra_flags->state_selra_flag }} id="check_state_selra_flag_1"
                                    name="check_state_selra_flag_1">
                            </div>
                        @endforeach
                        <div class="input-group mt-2" id="p21_tahap2" name="p21_tahap2" style="display: none">
                            <input class="form-check-input me-2" type="checkbox" value="1" id="p21_tahap2_1" name="p21_tahap2_1"
                            ><label class="fw-bold text-blue-dark">Tahap 2</label>
                        </div>
                    </form>
                    ...
                    <form id="upload-ketetapan-form" action="/upload-surat-ketetapan/{{ $id }}" method="POST"
                        ...
                        <!-- @csrf
                                ...
                                <div class="mt-2">
                                    <button id="btnImage" type="submit" class="btn btn-primary"> Upload Surat Ketetapan </button>
                                </div>
                        ...
                        <div class="row">
                            <div class="col-lg-10 col-md-10 col-sm-12 col-12 mt-3 mb-1">
                                <h4 class="text-blue-dark fw-bold">Upload Surat Ketetapan</h4>
                            </div>
                            <div class="col-lg-10  col-md-10 col-sm-10 col-10">
                                @if ($upload_surat_ketetapan == null)
                                    <input type="text" value="0" id="value_surat_ketetapan" hidden>
                                    <a class="btn btn-primary text-white font-weight-bold" id="surat_penghentian_15"
                                        name="surat_penghentian_15">KLIK DISINI</a>
                                @else
                                    ...
                                    <form action="/upload-surat-ketetapan/{{ $id }}" method="post">
                                        @method('DELETE')
                                        @csrf
                    ...
                    <div class="mt-3 alert alert-warning attentionBox" role="alert">
                        Selra menjadi POM/TNI dan SP2LID minimal <b>Sprinlidik</b>, sementara untuk P21, SP3, Diversi
                        minimal <b>SPDP</b> <br>
                        --------------------------------- <br>
    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            $('body').on('click', '.editBook', function() {
                ...
                $.ajax({
                    ...
                    //     $.get('{{ url('saksi/edit') }}/'+ book_id, function(data) {
                    //     // $('#name').val(data[0].name);
                    //     // $('#title').val(data.title);
                    //     // $('#author').val(data.author);
                    //     alert('sukses')
                })
                // $.get('{{ url('saksi/edit?id=') }}/'+ book_id, function(data) {
                //     alert('sukses')
                // });
            ...
            $(".btn-barang-bukti").click(function(e) {
                ...
                //         console.log(msg.success);
                //         $('.modal-saksi').append('<div class="alert alert-success alert-block" style="display: none;"><button type="button" class="close" data-dismiss="alert">×</button><strong class="success-msg"></strong></div>')
                //         $('.alert-block').css('display','block').append('<strong>'+'Sukses Menambah Saksi'+'</strong>');
                //     }else{
                //         $.each( msg.error, function( key, value ) {
                //         $('.'+key+'_err').text(value);
                //         });
        ...
        $(document).ready(function() {
            ...
            $('#selra_flag').change(function() {
                ...
                // $(".loaderbg").fadeIn();
                // $.ajax({
                //     url: "{{ route('update_selra') }}",
                //     type: 'POST',
                //     data: {
                //         _token: _token,
                //         accident_id: accident_id,
                //         selra_flag: selra_flag,
                //         state_selra_flag: state_selra_flag
                //     },
                //     success: function(data) {
                //         $("#loader").fadeOut();
                //         $(".loaderbg").fadeOut();
                //         alert("Status Selra Berhasil Diubah, Harap Upload Surat Ketetapan");
                if (selra_flag == 'S0101') {
                    $("#p21_tahap2").show();
            ...
            function addRowSuratTugas() {
                ...
                // '</td>'+
                // '<td>'+
                // '<button type="button" name="remove" class="btn btn-danger btn_remove_officer">X</button>'+
                // '</td>'+
                // '</tr>'
                // var tr = '<tr id="row_officer'+j+'" class="dynamic-added"><td> <input type="text" id="officer_id'+j+'" name="officer_id[]" placeholder="NRP - Nama Petugas" class="form-control officer_list" hidden /><input type="text" id ="officer'+j+'" name="officer[]" placeholder="NRP - Nama Petugas" class="form-control officer_list" autocomplete="off"/><div id="officer_list'+j+'"></div></td><td><button type="button" name="remove" id="'+j+'" class="btn btn-danger btn_remove_officer">X</button></td></tr>' ;
                // $('#add_surat_tugas').append(tr);
                $('#add_surat_tugas').append(div);
                //     $('#officer'+j+'').keyup(function(){
                //     var query = $(this).val();
                //     if(query != '')
                //     {
                //         var _token = $('input[name="_token"]').val();
                ...
                //         success:function(data){
                //         $('#officer_list'+j+'').fadeIn();
                //         $('#officer_list'+j+'').html(data);
                //         }
                //         });
                //     }
                // });
                ...
                //     $('#officer'+j+'').val($(this).text());
                //     $('#officer_id'+j+'').val($(this).attr('value'));
                //     $('#officer_list'+j+'').fadeOut();
                // });
            }
            ...
            function addRowEditSuratTugas() {
                ...
                // '</td>'+
                // '<td>'+
                // '<button type="button" name="remove" class="btn btn-danger btn_remove_officer_edit_surat_tugas">X</button>'+
                // '</td>'+
                // '</tr>'
                var div = '<div class="row add-row">' +
                    '<div class="input-group col-lg-11">' +
            ...
            function addRowSuratPenyelidikan() {
                ...
                // '</td>'+
                // '<td>'+
                // '<button type="button" name="remove" class="btn btn-danger btn_remove_officer_penyelidikan">X</button>'+
                // '</td>'+
                // '</tr>'
                var div = '<div class="row add-row">' +
                    '<div class="input-group col-lg-11">' +
            ...
            function addRowEditSuratPenyelidikan() {
                ...
                // '</td>'+
                // '<td>'+
                // '<button type="button" name="remove" class="btn btn-danger btn_remove_officer_edit_penyelidikan">X</button>'+
                // '</td>'+
                // '</tr>'
                var div = '<div class="row add-row">' +
                    '<div class="input-group col-lg-11">' +
            ...
            function addRowSuratPenyidikan() {
                ...
                // '</td>'+
                // '<td>'+
                // '<button type="button" name="remove" class="btn btn-danger btn_remove_officer_penyidikan">X</button>'+
                // '</td>'+
                // '</tr>'
                var div = '<div class="row add-row">' +
                    '<div class="input-group col-lg-11">' +
            ...
            function addRowEditSuratPenyidikan() {
                ...
                // '</td>'+
                // '<td>'+
                // '<button type="button" name="remove" class="btn btn-danger btn_remove_officer_edit_penyidikan">X</button>'+
                // '</td>'+
                // '</tr>'
                var div = '<div class="row add-row">' +
                    '<div class="input-group col-lg-11">' +
            ...
            function addRowSuratPenyitaan() {
                // var tr = '<tr>'+
                // '<td>'+
                // '<select id="officer_id_penyitaan[]" name="officer_id_penyitaan[]" class="form-control">'+
                // '<option value="" }}>Pilih Petugas</option>'+
                // '@foreach ($officer as $officers)'+
                // ' <option value="{{ $officers->id }}"'+
                // '{{ old('officer') == $officers->id ? 'selected' : '' }}>'+
                ...
                // '</td>'+
                // '<td>'+
                // '<button type="button" name="remove" class="btn btn-danger btn_remove_officer_penyitaan">X</button>'+
                // '</td>'+
                // '</tr>'
            ...
            function addRowEditSuratPenyitaan() {
                // var tr = '<tr>'+
                // '<td>'+
                // '<select id="edit_officer_surat_penyitaan[]" name="edit_officer_surat_penyitaan[]" class="form-control">'+
                // '<option value="" }}>Pilih Petugas</option>'+
                // '@foreach ($officer as $officers)'+
                // ' <option value="{{ $officers->id }}"'+
                // '{{ old('officer') == $officers->id ? 'selected' : '' }}>'+
                ...
                // '</td>'+
                // '<td>'+
                // '<button type="button" name="remove" class="btn btn-danger btn_remove_officer_edit_penyitaan">X</button>'+
                // '</td>'+
                // '</tr>'
                var div = '<div class="add-row mb-3">' +
                    '<div class="input-group">' +
            ...
            function addRowSuratPenyegelan() {
                ...
                // '</td>'+
                // '<td>'+
                // '<button type="button" name="remove" class="btn btn-danger btn_remove_officer_penyegelan">X</button>'+
                // '</td>'+
                // '</tr>'
                var div = '<div class="row add-row">' +
                    '<div class="input-group col-lg-11">' +
            ...
            function addRowEditSuratPenyegelan() {
                ...
                // '</td>'+
                // '<td>'+
                // '<button type="button" name="remove" class="btn btn-danger btn_remove_officer_edit_penyegelan">X</button>'+
                // '</td>'+
                // '</tr>'
                var div = '<div class="row add-row">' +
                    '<div class="input-group col-lg-11">' +
            ...
            //     $(document).on('click', '.btn_remove_officer', function(){
            //        var test=$('#add_surat_petugas tr').length;
            //        var button_id = $(this).attr("id");

            //         if(test==1){
            //             alert("you can not remove last row");
            //         }else{
            //             $('#row_officer'+button_id+'').remove();
            //         }
            ...
            //    k++;
            //    $('#dynamic_field_tujuan').append('<tr id="row_tujuan'+k+'" class="dynamic-added"><td><input type="text" name="tujuan[]" placeholder="Tujuan" class="form-control officer_list" /></td><td><button type="button" name="remove" id="'+k+'" class="btn btn-danger btn_remove_tujuan">X</button></td></tr>');
            // });

            // $(document).on('click', '.btn_remove_tujuan', function(){
            //    var button_id = $(this).attr("id");
            //    $('#row_tujuan'+button_id+'').remove();
            // });


            //     $('#officer0').keyup(function(){
            //        var query = $(this).val();
            //        if(query != '')
            //        {
            //         var _token = $('input[name="_token"]').val();
            ...
            //          data:{query:query, _token:_token},
            //          success:function(data){
            //           $('#officer_list0').fadeIn();
            //           $('#officer_list0').html(data);
            //          }
            //         });
            //        }
            //    });
            ...
            //    $(document).on('click', 'li', function(){
            //        $('#officer0').val($(this).text());
            //        $('#officer_id0').val($(this).text());
            //        $('#officer_list0').fadeOut();
            //    });

            // $('tbody').delegate('.officer_id,.officer','keyup',function(){
            //     var tr=$(this).parent().parent();
            //     var officer_id=tr.find('.officer_id').val();
            //     var officer=tr.find('.officer').val();
            ...
            //     $('#test1').append(tr);
            // };
            // $(document).on('click', '.test_remove', function(){
            //     var last=$('#test1 tr').length;
            //     if(last==1){
            //         alert("you can not remove last row");
            //     }
            //     else{
            ...
            //         }
            //     });
            //     var table = $('#table-saksi').DataTable({
            //         processing: true,
            //         serverSide: true,
            //         ajax: "{{ route('view_produktivitas_accident') }}",
            //         columns: [
            ...
            //     $.ajax({
            //         url: "{{ route('view_produktivitas_accident') }}",
            //         type:'GET',
            //         data: { _token:_token,accident_id:accident_id},
            //         success: function(data) {
            //             var obj = JSON.parse(data);
            //                 // $('#table-saksi tbody').prepend('<td scope="row">'+aaaaaa+'</td><td>'+data.name+'</td><td>'+data.gender+'</td><td>'+data.city, data.birt_date+'</td><td>'+data.religion+'</td><td>'+data.job+'</td><td>'+data.education+'</td><td>'+data.phone+'</td><td>'+data.citizen+'</td><td>'+data.address+'</td><td>'+test+'</td>');
            //                 $('#table-saksi tbody').append('<tr><td>'+data.name+'</td><td>'+data.gender+'</td><td>'+data.city+','+ data.birth_date+'</td><td>'+data.religion+'</td><td>'+data.job+'</td><td>'+data.education+'</td><td>'+data.phone+'</td><td>'+data.citizen+'</td><td>'+'test'+'</td></tr>');
            //                    $('#saksi-form')[0].reset();
            //            // append('<div class="alert alert-success alert-block" style="display: none;"><button type="button" class="close" data-dismiss="test">×</button><strong class="success-msg"></strong></div>');
            //             printMsg(data);
            //         }
